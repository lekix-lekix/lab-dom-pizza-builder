...
function renderPrice() {
  ...
  for(const property in state) {
    if (`${state[property]}` === "true") {
      newPrice += ingredients[`${property}`].price;
      document.querySelector(`.panel.price ul :nth-child(${i})`).style.display = 'block';
      i++;
    } else {
      document.querySelector(`.panel.price ul :nth-child(${i})`).style.display = 'none';
      i++;
    }
  }
